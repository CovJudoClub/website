...
- Hosted handoffs for member/payment flows: Coacha, British Judo membership, and initial-launch GoCardless-hosted Direct Debit setup via server-side Billing Request/Billing Request Flow creation.
...
- No secrets in git.
- No production payment/member-system changes without approval; GoCardless bank details must be collected only on GoCardless-hosted/approved surfaces, never directly on the CJC site.
